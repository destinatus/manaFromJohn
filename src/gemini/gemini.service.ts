...
export class GeminiService {
  ...
  private readonly supportedModels = ['gemini-pro', 'gemini-pro-vision', 'gemini-1.5-flash'];
  ...
  async createChatCompletion(request: ChatCompletionRequest) {
    try {
      ...
      if (systemMessage && firstUserMessageIndex !== -1) {
        const systemContent = typeof systemMessage.content === 'string' 
          ? systemMessage.content 
          : systemMessage.content.map(part => part.text || part.content || '').filter(Boolean).join('\n');
        
        const userMessage = messages[firstUserMessageIndex];
        const userContent = typeof userMessage.content === 'string'
          ? userMessage.content
          : userMessage.content.map(part => part.text || part.content || '').filter(Boolean).join('\n');
      ...
      const toolUseMatch = generatedText.match(/<(\w+)>([\s\S]*?)<\/\1>/);
      
      if (toolUseMatch) {
        // If it's a tool use response, return it as is
        return {
        ...
        // If no tool use pattern is found, wrap the response in attempt_completion tags
        generatedText = `<attempt_completion>\n<result>\n${generatedText}\n</result>\n</attempt_completion>`;
